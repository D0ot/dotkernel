...
void mms_free(mms_t *mms, BuddySystem *vbs, void *vaddr) {
    ...
    for(uint32_t i = 0; i < page_num; ++i) {
        mms_unmap(mms, (void*)((uint32_t)vaddr + (i << 12)));
    }
}
...
void mms_map(mms_t *mms, void *vaddr , void *paddr) {
    ...
    if(pde->p) {
        pte_paddr = (pte_t*)paging_paddr_in_pdet(pde);
    }else {
        pde->pde = 0;
        pte_paddr = (pte_t*)mms_alloc_phy(mms, 1);
        paging_set_pde_table_addr(pde, (uint32_t)pte_paddr);
        pde->p = 1;
        pde->rw = 1;
    }
    ...
    mms->pte_stub[0].rw = 1;
    mms->pte_stub[0].p = 1;
    paging_set_pte_addr(mms->pte_stub, (uint32_t)pte_paddr);
